...
altlist_new = np.arange(1,22.5,0.5)


##Interpolate data: (1441,43)


zh_avg_interp = np.ones((1441,43))*np.nan
zdr_avg_interp = np.ones((1441,43))*np.nan
kdp_avg_interp = np.ones((1441,43))*np.nan

zh_avg_lib_interp = np.ones((1441,43))*np.nan
zdr_avg_lib_interp = np.ones((1441,43))*np.nan
kdp_avg_lib_interp = np.ones((1441,43))*np.nan


for i in range(zh_avg_interp.shape[0]):
    
    zh_avg_interp[i,:] = np.interp(altlist_new,altlist, zh_avg[i,:])
    zdr_avg_interp[i,:] = np.interp(altlist_new,altlist, zdr_avg[i,:],)
    kdp_avg_interp[i,:] = np.interp(altlist_new,altlist, kdp_avg[i,:],)
    
    zh_avg_lib_interp[i,:] = np.interp(altlist_new,altlist, zh_avg_lib[i,:],)
    zdr_avg_lib_interp[i,:] = np.interp(altlist_new,altlist, zdr_avg_lib[i,:],)
    kdp_avg_lib_interp[i,:] = np.interp(altlist_new,altlist, kdp_avg_lib[i,:],)
...
zh2627new = zh_avg_interp[0:288,:]
zh2728new = zh_avg_interp[288:576,:]
zh2829new = zh_avg_interp[576:864,:]
...
plt.colorbar().set_label(label='dBZ',size=18,weight='bold')
plt.yticks(altlist_new[::6],altlist_new[::6])
plt.xticks(np.arange(0,288, step = 72),time_cut2728[::72])
#plt.ylim(altlist[0],altlist[18])
plt.tight_layout()
plt.show()
