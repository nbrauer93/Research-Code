...
#Select your time of choice (03 UTC on 2/19 is the default now; change the 1 to 2 if you want 06 UTC, etc.)
...
#cf = ax.contourf(lon[100:200], lat[100:200], isentrh[level, 100:200, 100:200], range(10, 106, 5), cmap=plt.cm.gist_earth_r, transform=ccrs.PlateCarree())

#cf = ax.contourf(lon,lat,isentspech[level,:,:],cmap = plt.cm.gist_earth_r, transform = ccrs.PlateCarree())
...
# Plot wind barbs
#ax.barbs(lon, lat, isentu[level, :, :], isentv[level, :, :], length=6,regrid_shape=20, transform=ccrs.PlateCarree())
ax.barbs(lon, lat, u_knots_nan[level, :, :], v_knots_nan[level, :, :], length=6,regrid_shape=20, transform=ccrs.PlateCarree())

# Make some titles
plt.title('296 K Pressure, Relative Humidity, Wind 2/19 06 UTC', size = 20)
